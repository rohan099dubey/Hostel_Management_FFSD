...
app.post("/services/problems/statusChange", async (req, res) => {
  ...
  try {
    console.log(`Status change request for problem ID: ${problemId}, new status: ${status}`);
    ...
    console.log(`Current problem status: ${problem.status}, changing to: ${status === "Resolved" ? "ToBeConfirmed" : status}`);
    ...
    if (status === "Resolved" && problem.status !== "ToBeConfirmed") {
      ...
      if (student && student.email) {
        console.log(`Preparing to send confirmation email to student: ${student.name} (${student.email})`);
        ...
        try {
          ...
          console.log(`Email configuration: ${process.env.EMAIL_HOST}, Port: ${process.env.EMAIL_PORT}, User: ${process.env.EMAIL_USER}`);
          ...
          console.log(`Sending email to: ${student.email}, Subject: Problem Resolution Confirmation Required`);

          // Use async/await and proper error handling for email sending
          try {
            const info = await transporter.sendMail(mailOptions);
            console.log(`Resolution confirmation email sent to ${student.email}, Message ID: ${info.messageId}`);
          } catch (sendError) {
      ...
      console.log(`No status change needed: Current=${problem.status}, Requested=${status}`);
    }

    await problem.save();
    console.log(`Problem ID: ${problemId} status successfully updated to ${problem.status}`);
...
app.post("/services/problems/student-confirmation", async (req, res) => {
  try {
    ...
    console.log(`Student confirmation for problem ID: ${problemId}, isResolved: ${isResolved}`);
    ...
    console.log(`Current problem status: ${problem.status}, studentStatus: ${problem.studentStatus}`);
    ...
    if (!student) {
      ...
      console.log(`Student found: ${student.name}, email: ${student.email || 'No email'}`);
    }
    ...
    if (isResolved === true) {
      ...
      console.log(`Finding admin users and warden for hostel: ${problem.hostel}`);
      ...
      if (staffRecipients.length > 0) {
        console.log(`Preparing to send staff notification to: ${staffRecipients.join(', ')}`);
        ...
        try {
          ...
          const staffEmailHtml = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e4e4e4; border-radius: 5px;">
              ...
              <div style="background-color: #f3f4f6; padding: 15px; border-radius: 5px; margin: 15px 0;">
                ...
                <p><strong>Room:</strong> ${problem.roomNo}</p>
                <p><strong>Student:</strong> ${student ? student.name : 'Unknown'} (${problem.studentId})</p>
                <p><strong>Description:</strong> ${problem.problemDescription}</p>
              </div>
          ...
          console.log(`Sending staff notification to: ${staffRecipients.join(', ')}`);
          const staffInfo = await transporter.sendMail(staffMailOptions);
          console.log(`Staff notification email sent, Message ID: ${staffInfo.messageId}`);
        } catch (emailError) {
      ...
      await problem.save();
      console.log(`Problem ID: ${problemId} marked as NOT resolved by student, returned to pending`);
